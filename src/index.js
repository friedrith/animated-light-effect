...
import laptop from "./assets/laptop-1x.png";
import product from "./assets/product-ui-1x.png";

if (document.getElementById("drawing")) {
  // check if drawing element is present
  if (SVG.supported) {
    // check if SVG is supported by this browser

    const draw = SVG("drawing").size(800, 600); // draw the main workspace

    // create a group to include laptop images
    const group = draw.group();
    const laptopBorder = draw.image(laptop, 800, 600);
    group.add(laptopBorder);
    const laptopContent = draw.image(product, 600, 600).move(100, -15);
